...
class GrownSamples(CompositeSystemReference):
    ...
    lab_id = Quantity(
        type=str,
        description='the Grown Sample used for the growth',
        a_tabular={
...
class MIST(Instrument, EntryData):
    ...
    m_def = Section(more={"label_quantity": "item"})
...
class MovpeExperimentIMEM(Experiment, EntryData, TableData):
    '''
    Class autogenerated from yaml schema.
    '''
    m_def = Section(
    ...
    def normalize(self, archive, logger: BoundLogger) -> None:
        '''
        The normalizer for the `MovpeExperimentIMEM` class.
        '''
        super(MovpeExperimentIMEM, self).normalize(archive, logger)
        ...
        if self.growth_run:
            if self.growth_run.reference:
                ...
                if grown_sample_ref.reference is None:
                    ...
                    setattr(grown_sample_ref, 'reference', f'../uploads/{archive.m_context.upload_id}/archive/{hash(archive.m_context.upload_id, filename)}#data')
