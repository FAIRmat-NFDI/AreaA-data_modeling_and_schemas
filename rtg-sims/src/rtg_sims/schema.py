...
class RTGSIMS(Measurement):
    ...
    techniques. For generating concentration depth profiles of elements SIMS is used
    as a standard method. Investigations close to the surface set typical range of
    5 nm to 20 µm and a lateral range from 50 µm up to 500 µm.
    The determination of concentrations (quantitative analyses) of all elements in
    the mass range from hydrogen to uran allow a valuation of the material quality
    of solids. In addition to depth profiles surface images with a lateral solution
    up to the µm range facilitates an topografical valuation of the material structure.
    Additionally, it is possible to determine the spacial element distribution and
    make it visible by imaging techniques. Using the Cameca spectrometers it is
    possible to determine elements in concentrations in ranges of ppb.
...
class RTGSIMSMeasurement(RTGSIMS, EntryData):
    ...
    def normalize(self, archive, logger):
        ...
        def parse_depthprofile_data(self):
            ...
            data = self.readlines()
            Zn_counter = 0
            # this is particular for FBH data where different Zn ions were measured
            element = None
            depth_profiles_qual = []
