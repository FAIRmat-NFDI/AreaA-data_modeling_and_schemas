...
class RawFileDepositionControl(EntryData):
    m_def = Section(
        a_eln=None, categories=[IKZMOVPE1Category], label="Raw File Deposition Control"
    )
...
class ParserMovpe1DepositionControlIKZ(MatchingParser):
    ...
    def parse(self, mainfile: str, archive: EntryArchive, logger) -> None:
        ...
        ]  # this line is not used now, tabular data reads the raw file columns
        ...
        for movpe_sample_index, _ in enumerate(dep_control["Sample ID"]):
            if (
                dep_control["Sample ID"][movpe_sample_index]
                == precursors["Sample ID"][movpe_sample_index]
            ):
                continue
            else:
                logger.error(
                    f"Sample ID no.{movpe_sample_index} in 'deposition control' and 'precursors' Excel sheets are not equal. "
        ...
        precursors_filename = (
            f"{precursors['Sample ID'][0]}_precursors_preparation.archive.{filetype}"
        )
        ...
        while True:
            ...
            if toc - tic > 200:
                logger.warning(
                    "Some rows of 'deposition control' Excel file were not parsed. Please check the file and try again."
                )
                break
        ...
        while True:
            ...
            # checking if all entries are properly indexed
            if search_precursor_preparation.pagination.total == len(
                dep_control["Sample ID"]
            ):
                break
            if search_precursor_preparation.pagination.total > len(
                dep_control["Sample ID"]
        ...
        if search_dep_control.pagination.total == len(dep_control["Sample ID"]):
            for deposition_control_entry in search_dep_control.data:
                ...
                for row_index, row_id in enumerate(dep_control["Sample ID"]):
                    if (
                        deposition_control_entry["results"]["eln"]["lab_ids"][0]
                        == row_id
                    ):
                        constant_parameters_id = dep_control["Constant Parameters ID"][
                            row_index
                        ]
                for precursor_preparation_entry in search_precursor_preparation.data:
                    if (
                        deposition_control_entry["results"]["eln"]["lab_ids"][0]
                        == precursor_preparation_entry["results"]["eln"]["lab_ids"][0]
                    ):
                        precursor_preparation_archive = precursor_preparation_entry[
                            "entry_id"
                        ]
