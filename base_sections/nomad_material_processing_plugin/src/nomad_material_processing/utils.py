...
def _get_reference(upload_id, entry_id):
    return f'../uploads/{upload_id}/archive/{entry_id}#data'


def _get_entry_id_from_file_name(file_name, archive):
    from nomad.utils import hash
    return hash(archive.metadata.upload_id, file_name)


def _create_archive(entity, archive, file_name):
    import json
...
def create_archive(entity, archive, file_name) -> str:
    _create_archive(entity, archive, file_name)
    return _get_reference(
        archive.metadata.upload_id,
        _get_entry_id_from_file_name(file_name, archive)
    )
